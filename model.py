...
def load_dataset_from_csv(csv_path, image_size=(28, 28), test_size=0.2, random_state=42):
        data=pd.read_csv('/Users/purvimalhotra/Developer/ccode/IEEE_ml1/data.csv')
...
X_train_tensor=torch.FloatTensor(X_train_norm).unsqueeze(1)  
...
input_dim=X_train_norm.shape[1] * X_train_norm.shape[2]  
...
def load_model_and_predict(model_path,input_image):
    ...
    loaded_model=FashionClassifier(input_dim=input_dim,hidden_dim1=128,hidden_dim2=64,output_dim=n_classes)
    ...
    input_tensor=torch.FloatTensor(input_image)
